...
const CreateQuiz = () => {
  const [categories, setCategories] = useState([]);  // Initialisiere als leeres Array
  const [newCategory, setNewCategory] = useState(''); // Initialisiert als leerer String
  const [selectedCategory, setSelectedCategory] = useState(''); // Initialisiert als leerer String
  ...
  const fetchCategories = async () => {
    try {
      ...
      setCategories(data.categories || []);  // Fallback auf leeres Array, wenn undefined
    } catch (error) {
      console.error('Fehler beim Laden der Kategorien:', error);
      setCategories([]);  // Setze categories auf ein leeres Array bei Fehler
    }
  };
  ...
  const handleAddCategory = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/add-category', {
        ...
        body: JSON.stringify({ category: newCategory, createdBy: email }),
      });
      ...
      if (data.error) {
        ...
        const newCategories = [...categories, { name: newCategory, questions: [] }];
